...
function init(argv) {
  ...
  try {
    (0, _child_process.execSync)('git clone -b ' + branch + ' ' + repoUrl + ' ' + dirName, { stdio: 'inherit' });
  } catch (err) {
  ...
  if (tag) {
    ...
    try {
      (0, _child_process.execSync)('cd ' + dirName + ' && git checkout tags/' + tag + ' -b ' + tag, { stdio: 'inherit' });
    } catch (err) {
