...
function exists(searchPath) {
  try {
    _fsExtra2.default.statSync(searchPath);
    return true;
  } catch (e) {
    return false;
  }
...
function isEmptyOrMissing(searchPath) {
  var stat = void 0;
  try {
    stat = _fsExtra2.default.statSync(searchPath);
  } catch (e) {
    return true;
  }
  if (stat.isDirectory()) {
    var items = _fsExtra2.default.readdirSync(searchPath);
    return !items || !items.length;
  }
  var file = _fsExtra2.default.readFileSync(searchPath);
...
function getDirectories(dir) {
  try {
    var files = _fsExtra2.default.readdirSync(dir).filter(function (file) {
      return _fsExtra2.default.statSync(_path2.default.join(dir, file)).isDirectory();
    });
    return files;
  } catch (e) {
...
function getFiles(dir) {
  try {
    var files = _fsExtra2.default.readdirSync(dir).filter(function (file) {
      return _fsExtra2.default.statSync(_path2.default.join(dir, file)).isFile();
    });
    return files;
  } catch (e) {
...
function getJSONFromFile(file) {
  try {
    return _fsExtra2.default.readJSONSync(file);
  } catch (error) {
