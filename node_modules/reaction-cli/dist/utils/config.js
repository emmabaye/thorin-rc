...
function initGlobalConfig() {
  ...
  if ((0, _fs.isEmptyOrMissing)(idFile)) {
    try {
      _fsExtra2.default.writeJSONSync(idFile, { id: _uuid2.default.v1(), since: Date.now() });
    } catch (error) {
      _logger2.default.error('Error creating Reaction CLI configs');
      process.exit(1);
    }
  } else {
    var idFileContent = _fsExtra2.default.readJsonSync(idFile);

    if (!idFileContent.since) {
      _fsExtra2.default.writeJsonSync(idFile, _extends({}, idFileContent, { since: Date.now() }));
    }
  }
...
function getUserId() {
  ...
  try {
    ...
    _logger2.default.error('Error reading Reaction config file: ' + _logger2.default.magenta(idFile));
    process.exit(1);
  }
}
